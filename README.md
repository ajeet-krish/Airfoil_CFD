...
Automated CFD analysis of NACA 4-digit airfoils across multiple angles of attack using SU2 RANS (Spalart-Allmaras) on high-quality C-grid meshes with structured boundary layers. Includes airfoil shape optimization via CST parameterization + NeuralFoil surrogate + SU2 verification, and structural analysis of the optimized wing under CFD aerodynamic loads using FElupe FEA. Field visualizations rendered in ParaView; convergence and aggregate plots generated via matplotlib.

## Features

- **Modular Architecture**: Decoupled `physics/` modules for geometry, meshing, solving, post-processing, optimization, and structural analysis
...
- **Structural Analysis**: CFD pressure mapped to 3D FEA wing via FElupe (linear elastic, von Mises stress, safety factor)
...
# Run structural analysis of optimized wing (~6 sec)
...
│       └── optimized/          Optimized airfoil + FEA images
├── output/                     Simulation artifacts (gitignored)
└── output_optimized/           Optimization + FEA artifacts (gitignored)
```
